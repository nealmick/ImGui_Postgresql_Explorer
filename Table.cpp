...
void Table::loadTableData(const std::string& tableName, int offset) {
    ...
    std::string dataQuery = columns.empty() ? 
        buildInitialQuery(offset) : buildFilteredQuery();
...
std::string Table::buildInitialQuery(int offset) const {
    return "SELECT * FROM \"" + currentTable + 
           "\" ORDER BY 1 LIMIT " + 
           std::to_string(rowsPerPage) + 
...
void Table::renderTableCell(int row, int col) {
    ...
    if (ImGui::Selectable(cellId.c_str(), false, 
...
void Table::renderTableCellEdit(int row, int col) {
    ...
    bool valueChanged = ImGui::InputTextMultiline(
        editId.c_str(), 
        editBuffer, 
        sizeof(editBuffer), 
...
bool Table::isFilterActive() const {
    return std::any_of(columnFilters.begin(), columnFilters.end(),
        [](const std::string& filter) { return !filter.empty(); });
}

void Table::renderPagination() {
    ImGui::PushStyleVar(ImGuiStyleVar_WindowBorderSize, 0.0f);
    ImGui::BeginChild("##Pagination", ImVec2(0, 30), false, 
...
void Table::renderPaginationControls() {
    ...
    ImGui::Text("Page %d (rows %d-%d)", 
        (currentOffset / rowsPerPage) + 1,
...
void Table::setupTableFlags(ImGuiTableFlags& flags) const {
...
bool Table::shouldShowFilter(size_t colIndex) const {
    return activeFilterColumn == colIndex;
}
...
void Table::renderFilterInput(size_t colIndex) {
    ...
    if (ImGui::InputText(filterId.c_str(), filterBuffer, sizeof(filterBuffer), 
...
void Table::renderHeaderCell(size_t colIndex) {
    ...
    ImGui::PushStyleVar(ImGuiStyleVar_FramePadding, ImVec2(2, 2));
    if (ImGui::Selectable(headerText.c_str(), false, 
